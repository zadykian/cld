...
16. Resume (#26): `resume [-n NAME] [SESSION]` joins the commands of decision 3. It brings back a
    ...
    8. tmux ends a command at an argv word ending in `;` (see Findings), and SESSION can end in
       ...
       does goes to tmux with a `\` before the `;`, which tmux drops, so that tmux gets the
       directory, and claude every word, as cld has them. Before, `new` failed in a directory
       ending in `;`, with status 1 and no session. The test harness's outer tmux does the same
       for the command it starts and its directory.
